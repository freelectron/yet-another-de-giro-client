...
class DeGiroSession:
    ...
    def __init__(
        ...
        self.browser_session = getattr(webdriver, browser_name)(service=self.web_service)
        self.requests_session = requests.session()
        self.requests_session.headers.update(requests_headers) if requests_headers else None
    ...
    def connect(self, username: str, password: str):
        ...
        sleep(0.5)
        self.browser_session.find_element(by=By.ID, value="username").send_keys(username)
        self.browser_session.find_element(by=By.ID, value="password").send_keys(password)
        self.browser_session.find_element(by=By.NAME, value="loginButtonUniversal").click()

        for cookie in self.browser_session.get_cookies():
            c = {cookie['name']: cookie['value']}
            self.requests_session.cookies.update(c)

        self.session_id = [cookie['value'] for cookie in self.browser_session.get_cookies() if
                           cookie.get('name') == "JSESSIONID"].pop()

    def get_portfolio_performance_results(self, date: Date) -> pd.DataFrame:
        """
        Get portfolio returns in % and monetary units.
        """
        date_str = date.strftime(self.date_string_format)
        query_url = f"{self.portfolio_url}/csv?" \
        ...
        return pd.read_csv(StringIO(str(response.content.decode('utf-8'))))
